...
int send_reply(int ctl_conn, const char * send_buf, int len){
    if (send(ctl_conn, send_buf, len, 0) < 0){
        myftp_log(MYFTP_LOG_ERROR, "Failed to send reply to server. Terminating...\n");
        return -1;
    }else{
        return 0;
    }
...
int send_reply(int ctl_conn, const char * send_buf){
    return send_reply(ctl_conn, send_buf, strlen(send_buf));
}
...
int store_file(int ctl_conn, int data_conn, struct sockaddr_in * data_sock_addr, const char * filename){
    ...
    while(true){
        ...
        if (len > 0){
            send_reply(data_send_conn, fread_buf, len);
        }else{
            break;
        }
    }
